...
test("Codex client resolves OAuth account, roots, headers, and API errors", async () => {
  ...
  assert.equal(request?.init?.redirect, "error", "authenticated requests must not follow unchecked redirects");
...
test("Codex client preserves cancellation and never sends an already aborted request", async () => {
  let requests = 0;
  const client = new CodexApiClient({
  ...
  aborted.abort(new DOMException("Test cancelled before sending", "AbortError"));
  await assert.rejects(client.post("test", {}, aborted.signal), (error) => error === aborted.signal.reason);
  assert.equal(requests, 0);
  const active = new AbortController();
  const pending = client.post("test", {}, active.signal);
  active.abort(new DOMException("Test cancelled in flight", "AbortError"));
  await assert.rejects(pending, (error) => error === active.signal.reason);
  assert.equal(requests, 1);
});

test("codex_image preserves a destination created while generation was in flight", async () => {
  const temporary = await mkdtemp(join(tmpdir(), "pi-codex-image-conflict-"));
  const destination = join(temporary, "generated.png");
  const originalFetch = globalThis.fetch;
  globalThis.fetch = async () => {
    await writeFile(destination, "asset created during generation");
    return new Response(JSON.stringify({ data: [{ b64_json: Buffer.from("generated bytes").toString("base64") }] }));
  };
  try {
    const tool = toolRegistry(registerCodexImageTool);
    await assert.rejects(
      tool.execute("image-conflict", { prompt: "Test image", output_path: "generated.png" }, undefined, undefined, context(temporary)),
      /Refusing to overwrite existing image/,
    );
    assert.equal(await readFile(destination, "utf8"), "asset created during generation");
  } finally {
...
test("Codex usage refresh times out stalled requests instead of syncing forever", async () => {
  ...
  try {
    ...
    await assert.rejects(handle.refreshUsage(ctx, true), { name: "TimeoutError" });
...
async function codexUsageTimerHarness() {
  const temporary = await mkdtemp(join(tmpdir(), "pi-codex-stale-timers-"));
  const authPath = join(temporary, "auth.json");
  await writeFile(authPath, "{}");
  const handlers = new Map<string, (event: any, ctx: ExtensionContext) => unknown>();
  const commands = new Map<string, Parameters<ExtensionAPI["registerCommand"]>[1]>();
  const timeouts: CodexUsageTestTimer[] = [];
  const intervals: CodexUsageTestTimer[] = [];
  const originals = {
  ...
  const schedule = (timers: CodexUsageTestTimer[]) => (cb: () => void, delay: number) => {
    const timer = { delay, cb, cleared: false, fired: false, unref() {} };
    timers.push(timer);
    return timer as unknown as ReturnType<typeof setTimeout>;
  };
  const clear = (timer: unknown) => {
    if (timer) (timer as CodexUsageTestTimer).cleared = true;
  };
  globalThis.setTimeout = schedule(timeouts) as unknown as typeof setTimeout;
  globalThis.setInterval = schedule(intervals) as unknown as typeof setInterval;
  globalThis.clearTimeout = clear as typeof clearTimeout;
  globalThis.clearInterval = clear as typeof clearInterval;
  Date.now = () => state.now;
  globalThis.fetch = async () => {
    state.usageFetches += 1;
    return new Response(JSON.stringify({
      rate_limit: {
        primary_window: {
          used_percent: 10,
          limit_window_seconds: 5 * 60 * 60,
          reset_at: Math.floor(state.now / 1000) + 90 * 60,
        },
      },
    }));
  };
  const handle = registerCodexUsageAndFast({
    ...
    getConfig: () => {
      if (state.configUnavailable) throw new Error("Test configuration unavailable");
      return { ...DEFAULT_CODEX_API_CONFIG, usagePollInterval: 1 };
    },
    updateConfig: () => {},
  }, { authPath });
  const makeContext = () => {
    const statuses: Array<string | undefined> = [];
    const ctx = context("/local/workspace");
    ctx.modelRegistry.isUsingOAuth = () => state.oauthAvailable;
    ctx.ui = {
      setStatus: (_key: string, value: string | undefined) => statuses.push(value),
      theme: plainTheme,
      notify() {},
  ...
  const activeTimers = (kind: "poll" | "countdown") =>
    (kind === "poll" ? timeouts : intervals).filter((timer) => !timer.cleared && !timer.fired);
  const tick = (kind: "poll" | "countdown") => {
    const timers = activeTimers(kind);
    assert.equal(timers.length, 1, `expected one active ${kind} timer`);
    if (kind === "poll") timers[0].fired = true;
    timers[0].cb();
  };
  return {
    state, handle, handlers, commands, makeContext, activeTimers, tick,
    async close() {
      try {
        // Use a live cleanup context even when a regression assertion failed
        // before the test could shut down its intentionally invalidated ctx.
        await handlers.get("session_shutdown")!({}, makeContext().ctx);
      } finally {
...
test("Codex usage timers tolerate stale context getters and shutdown", async (t) => {
  for (const kind of ["poll", "countdown"] as const) {
    for (const property of ["model", "modelRegistry", "ui"] as const) {
      await t.test(`${kind}: stale ctx.${property}`, async () => {
        const h = await codexUsageTimerHarness();
        const { ctx } = h.makeContext();
        try {
          await h.handlers.get("session_start")!({}, ctx);
          assert.equal(h.state.usageFetches, 1);
          assert.equal(h.activeTimers("countdown").length, 1);
          Object.defineProperty(ctx, property, {
            configurable: true,
            get() { throw new Error("This extension ctx is stale after session replacement or reload."); },
          });
          // Exercise the status-clear path separately from the model guards.
          if (property === "ui") h.state.oauthAvailable = false;
          assert.doesNotThrow(() => h.tick(kind));
          await new Promise<void>((resolve) => setImmediate(resolve));
          assert.equal(h.state.usageFetches, 1, "stale ticks must not fetch usage");
          assert.equal(h.activeTimers("countdown").length, 0);
          assert.equal(h.activeTimers("poll").length, 1, "polling can recover with a fresh context");
          assert.doesNotThrow(() => h.handle.refreshStatus(ctx));
          assert.doesNotThrow(() => h.handlers.get("session_shutdown")!({}, ctx));
          assert.equal(h.activeTimers("poll").length, 0);
          assert.equal(h.activeTimers("countdown").length, 0);
        } finally {
          await h.close();
        }
...
test("Codex usage timer boundaries contain unexpected synchronous errors", async (t) => {
  for (const kind of ["poll", "countdown"] as const) {
    await t.test(kind, async () => {
      ...
      try {
        ...
        const now = Date.now;
        try {
          // Neither the poll's age check nor countdown formatting is a ctx
          // getter: this verifies the timer boundary, not just safe predicates.
          Date.now = () => { throw new Error("Test clock unavailable"); };
          assert.doesNotThrow(() => h.tick(kind));
        } finally {
          Date.now = now;
        }
        assert.equal(h.activeTimers("poll").length, 1);
        if (kind === "countdown") assert.equal(h.activeTimers("countdown").length, 0);
        h.state.now += 60_000;
        h.tick("poll");
        await new Promise<void>((resolve) => setImmediate(resolve));
        assert.equal(h.state.usageFetches, 2, "a later tick must still refresh successfully");
        assert.equal(h.activeTimers("countdown").length, 1);
      } finally {
        await h.close();
      }
...
test("Codex usage poll scheduling errors stay contained and a live event can restart polling", async () => {
  const h = await codexUsageTimerHarness();
  const { ctx } = h.makeContext();
  try {
    await h.handlers.get("session_start")!({}, ctx);
    h.state.configUnavailable = true;
    assert.doesNotThrow(() => h.tick("poll"));
    assert.equal(h.activeTimers("poll").length, 0);
    h.state.configUnavailable = false;
    h.handle.refreshStatus(ctx);
    assert.equal(h.activeTimers("poll").length, 1);
  } finally {
    await h.close();
  }
...
test("Codex usage ignores cancelled timer callbacks after shutdown and session replacement", async () => {
  const h = await codexUsageTimerHarness();
  const old = h.makeContext();
  try {
    await h.handlers.get("session_start")!({}, old.ctx);
    const oldPoll = h.activeTimers("poll")[0];
    const oldCountdown = h.activeTimers("countdown")[0];
    h.handlers.get("session_shutdown")!({}, old.ctx);
    const oldStatusCount = old.statuses.length;
    // Deliberately invoke cancelled callbacks to simulate already queued work.
    assert.doesNotThrow(() => oldPoll.cb());
    assert.doesNotThrow(() => oldCountdown.cb());
    assert.equal(h.activeTimers("poll").length, 0);
    assert.equal(h.activeTimers("countdown").length, 0);
    assert.equal(old.statuses.length, oldStatusCount);

    const fresh = h.makeContext();
    await h.handlers.get("session_start")!({}, fresh.ctx);
    const freshPoll = h.activeTimers("poll")[0];
    const freshCountdown = h.activeTimers("countdown")[0];
    const freshStatusCount = fresh.statuses.length;
    oldPoll.cb();
    oldCountdown.cb();
    assert.deepEqual(h.activeTimers("poll"), [freshPoll]);
    assert.deepEqual(h.activeTimers("countdown"), [freshCountdown]);
    assert.equal(fresh.statuses.length, freshStatusCount);
    h.state.now += 60_000;
    h.tick("poll");
    await new Promise<void>((resolve) => setImmediate(resolve));
    assert.equal(h.state.usageFetches, 3);
    assert.match(fresh.statuses.at(-1) ?? "", /Codex 5h 90%/);
    assert.equal(old.statuses.length, oldStatusCount);
  } finally {
    await h.close();
  }
...
test("Codex lifecycle completions cannot restart usage polling after shutdown", async (t) => {
  for (const event of ["session_start", "model_select"]) {
    await t.test(event, async () => {
      const h = await codexUsageTimerHarness();
      const old = h.makeContext();
      const entered = codexDeferred<void>();
      const reply = codexDeferred<Response>();
      globalThis.fetch = async () => { entered.resolve(); return reply.promise; };
      const starting = h.handlers.get(event)!({}, old.ctx);
      try {
        await entered.promise;
        h.handlers.get("session_shutdown")!({}, old.ctx);
        const statusCount = old.statuses.length;
        reply.resolve(new Response(JSON.stringify({ rate_limit: { primary_window: { used_percent: 90 } } })));
        await starting;
        assert.equal(h.activeTimers("poll").length, 0, "late event continuation must not restart polling");
        assert.equal(h.activeTimers("countdown").length, 0);
        assert.deepEqual(h.handle.getSnapshots(), []);
        assert.equal(old.statuses.length, statusCount);
      } finally {
...
test("Codex delayed OAuth resolution cannot reactivate a replaced session", async () => {
  const h = await codexUsageTimerHarness();
  const old = h.makeContext();
  const auth = codexDeferred<{ ok: true; apiKey: string }>();
  old.ctx.modelRegistry.getApiKeyAndHeaders = async () => auth.promise;
  const pending = h.handle.refreshUsage(old.ctx, true).catch((error: unknown) => error);
  try {
    h.handlers.get("session_shutdown")!({}, old.ctx);
    const fresh = h.makeContext();
    fresh.ctx.modelRegistry.getApiKeyAndHeaders = async () => ({ ok: true, apiKey: jwt("acct-b") });
    await h.handlers.get("session_start")!({}, fresh.ctx);
    const statusCount = fresh.statuses.length;
    auth.resolve({ ok: true, apiKey: jwt("acct-a") });
    await pending;
    assert.equal(h.state.usageFetches, 1, "old OAuth completion must not send another usage request");
    assert.equal(fresh.statuses.length, statusCount);
    assert.equal(h.handle.getSnapshots()[0]?.primary?.usedPercent, 10);
    await h.handle.refreshUsage(old.ctx, true).catch(() => {});
    assert.equal(h.state.usageFetches, 1, "a late tool completion must not re-adopt its old context");
  } finally {
    auth.resolve({ ok: true, apiKey: jwt("acct-a") });
    await pending;
    await h.close();
...
test("Codex late tool contexts with stale getters cannot replace the live polling context", async () => {
  const h = await codexUsageTimerHarness();
  try {
    const fresh = h.makeContext();
    await h.handlers.get("session_start")!({}, fresh.ctx);
    const unseen = h.makeContext();
    Object.defineProperty(unseen.ctx, "model", {
      get() { throw new Error("This extension ctx is stale after session replacement or reload."); },
    });
    await h.handle.refreshUsage(unseen.ctx, true).catch(() => {});
    h.state.now += 60_000;
    h.tick("poll");
    await new Promise<void>((resolve) => setImmediate(resolve));
    assert.equal(h.state.usageFetches, 2);
    assert.match(fresh.statuses.at(-1) ?? "", /Codex 5h 90%/);
    assert.deepEqual(unseen.statuses, []);
  } finally {
    await h.close();
  }
});

test("Codex account checks do not lose a newer model context behind an older check", async () => {
  const h = await codexUsageTimerHarness();
  const old = h.makeContext();
  const auth = codexDeferred<{ ok: true; apiKey: string }>();
  old.ctx.modelRegistry.getApiKeyAndHeaders = async () => auth.promise;
  const starting = h.handlers.get("session_start")!({}, old.ctx);
  try {
    const fresh = h.makeContext();
    const selecting = h.handlers.get("model_select")!({}, fresh.ctx);
    await new Promise<void>((resolve) => setImmediate(resolve));
    const fetchedBeforeOldCompletion = h.state.usageFetches;
    auth.resolve({ ok: true, apiKey: jwt("acct-a") });
    await Promise.all([starting, selecting]);
    assert.equal(fetchedBeforeOldCompletion, 1, "new context must not wait for an obsolete account check");
    assert.match(fresh.statuses.at(-1) ?? "", /Codex 5h 90%/);
    assert.equal(h.state.usageFetches, 1);
  } finally {
    auth.resolve({ ok: true, apiKey: jwt("acct-a") });
...
test("Codex header refresh failures cannot revive an old context", async () => {
  const h = await codexUsageTimerHarness();
  const old = h.makeContext();
  const auth = codexDeferred<{ ok: true; apiKey: string }>();
  try {
    await h.handlers.get("session_start")!({}, old.ctx);
    old.ctx.modelRegistry.getApiKeyAndHeaders = async () => auth.promise;
    h.handlers.get("after_provider_response")!({ headers: { "x-codex-primary-used-percent": "99" } }, old.ctx);
    h.handlers.get("session_shutdown")!({}, old.ctx);
    const fresh = h.makeContext();
    await h.handlers.get("session_start")!({}, fresh.ctx);
    const oldStatusCount = old.statuses.length;
    auth.reject(new Error("Delayed credential failure"));
    await new Promise<void>((resolve) => setImmediate(resolve));
    assert.equal(old.statuses.length, oldStatusCount);
    h.state.now += 60_000;
    h.tick("poll");
    await new Promise<void>((resolve) => setImmediate(resolve));
    assert.equal(h.state.usageFetches, 3, "polling must still belong to the replacement context");
    assert.match(fresh.statuses.at(-1) ?? "", /Codex 5h 90%/);
  } finally {
    auth.reject(new Error("Test cleanup"));
    await new Promise<void>((resolve) => setImmediate(resolve));
    await h.close();
  }
});

test("Codex redeem does not consume a credit after the account changes during confirmation", async () => {
  const h = await codexUsageTimerHarness();
  const old = h.makeContext();
  const confirming = codexDeferred<void>();
  const choice = codexDeferred<string | undefined>();
  let consumed = 0;
  const fetchUsage = globalThis.fetch;
  globalThis.fetch = async (input, init) => {
    if (init?.method === "POST") { consumed += 1; return new Response("{}"); }
    if (String(input).includes("rate-limit-reset-credits")) {
      return new Response(JSON.stringify({ available_count: 1, credits: [{ id: "credit-a", status: "available" }] }));
    }
    return fetchUsage(input, init);
  };
  let redeem: Promise<unknown> | undefined;
  try {
    await h.handlers.get("session_start")!({}, old.ctx);
    (old.ctx as any).hasUI = true;
    old.ctx.ui.select = async () => { confirming.resolve(); return choice.promise; };
    redeem = h.commands.get("codex-redeem")!.handler("", old.ctx as any);
    await confirming.promise;
    const fresh = h.makeContext();
    fresh.ctx.modelRegistry.getApiKeyAndHeaders = async () => ({ ok: true, apiKey: jwt("acct-b") });
    await h.handle.refreshUsage(fresh.ctx, true);
    choice.resolve("Yes");
    await redeem;
    assert.equal(consumed, 0, "confirmation for an obsolete account must not authorize a POST");
    assert.match(fresh.statuses.at(-1) ?? "", /Codex 5h 90%/);
  } finally {
...
test("Codex redeem rechecks credentials after confirmation even before the auth watcher runs", async () => {
  const h = await codexUsageTimerHarness();
  const { ctx } = h.makeContext();
  let token = jwt("acct-a");
  ctx.modelRegistry.getApiKeyAndHeaders = async () => ({ ok: true, apiKey: token });
  (ctx as any).hasUI = true;
  ctx.ui.select = async () => { token = jwt("acct-b"); return "Yes"; };
  let consumed = 0;
  const fetchUsage = globalThis.fetch;
  globalThis.fetch = async (input, init) => {
    if (init?.method === "POST") { consumed += 1; return new Response("{}"); }
  ...
  try {
    await h.handlers.get("session_start")!({}, ctx);
    await h.commands.get("codex-redeem")!.handler("", ctx as any);
    assert.equal(consumed, 0, "an old account confirmation must not survive a credential switch");
  } finally {
    await h.close();
  }
});
